class Search extends HTMLElement {
        ...
        return new Promise((resolve) => {
            window.callback = (res) => {
                ...
                for (var i = 0; i < res[1].length; i++) {
                    if (res[1][i].toLowerCase() === search.toLowerCase()) continue;
                    suggestions.push(res[1][i]);
                }
            ...
            script.src = `http://suggestqueries.google.com/complete/search?client=firefox&q=${search}&callback=callback&hl=en`
    ...
    #onInput = async () => {
        ...
        if (oq.search && suggestions.length < CONFIG.suggestionLimit) {
            try {
                const res = await Search.#googleSuggests(oq.search);
